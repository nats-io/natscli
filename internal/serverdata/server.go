...
func (s *Server) Varz(opts server.VarzEventOptions) ([]*server.ServerAPIVarzResponse, error) {
	subj := serverSubject(opts.EventFilterOptions, "VARZ")
	return doRequest[server.ServerAPIVarzResponse](s, opts, subj)
}
...
func (s *Server) Healthz(opts server.HealthzEventOptions) ([]*server.ServerAPIHealthzResponse, error) {
	subj := serverSubject(opts.EventFilterOptions, "HEALTHZ")
	return doRequest[server.ServerAPIHealthzResponse](s, opts, subj)
}
...
func (s *Server) Ipqueuesz(opts server.IpqueueszEventOptions) ([]*server.ServerAPIpqueueszResponse, error) {
	return doRequest[server.ServerAPIpqueueszResponse](s, opts, "$SYS.REQ.SERVER.PING.IPQUEUESZ")
}

func (s *Server) Raftz(opts server.RaftzEventOptions) ([]*server.ServerAPIRaftzResponse, error) {
	return doRequest[server.ServerAPIRaftzResponse](s, opts, "$SYS.REQ.SERVER.PING.RAFTZ")
}

func (s *Server) Profilez(opts server.ProfilezEventOptions) ([]*ProfilezResponse, error) {
	return doRequest[ProfilezResponse](s, opts, "$SYS.REQ.SERVER.PING.PROFILEZ")
}
...
// serverSubject returns the appropriate $SYS subject for a request.
// When Name is set with ExactMatch, targets that specific server directly.
// Otherwise broadcasts via PING.
func serverSubject(f server.EventFilterOptions, endpoint string) string {
	if f.Name != "" && f.ExactMatch {
		return fmt.Sprintf("$SYS.REQ.SERVER.%s.%s", f.Name, endpoint)
	}
	return fmt.Sprintf("$SYS.REQ.SERVER.PING.%s", endpoint)
}
