...
// Profilez reads profilez data from the archive
func (a *Archive) Profilez(opts server.ProfilezEventOptions) ([]*ProfilezResponse, error) {
	var results []*ProfilezResponse
	_, err := archive.EachClusterServerArtifact(a.reader, archive.TagServerProfile(),
		func(_ *archive.Tag, _ *archive.Tag, cbError error, r *ProfilezResponse) error {
			if cbError != nil || !matchesFilter(r.Server, &opts.EventFilterOptions) {
				return nil
			}
			results = append(results, r)
			return nil
