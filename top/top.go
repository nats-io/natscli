...
var (
	...
	standardHeaders = []any{"SUBS", "PENDING", "MSGS_TO", "MSGS_FROM", "BYTES_TO", "BYTES_FROM", "LANG", "VERSION", "UPTIME", "LAST_ACTIVITY"}
...
func generateParagraphPlainText(engine *Engine, stats *Stats, lookupDNS bool, rawBytes bool) string {
	...
	header := make([]any, 0) // Dynamically add columns and padding depending
...
func generateParagraphCSV(engine *Engine, stats *Stats, delimiter string, lookupDNS bool, rawBytes bool) string {
	...
	header := make([]any, 0) // Dynamically add columns
	connHeader := ""
