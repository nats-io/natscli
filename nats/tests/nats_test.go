...
func TestCLIPubSendOnNewline(t *testing.T) {
	...
	t.Run("Publish with body argument", func(t *testing.T) {
		out := runNatsCli(t, fmt.Sprintf("--server='%s' pub --send-on newline test x", srv.ClientURL()))
	...
	t.Run("Publish with body argument and --quiet", func(t *testing.T) {
		out := runNatsCli(t, fmt.Sprintf("--server='%s' pub --send-on newline -q test x", srv.ClientURL()))
	...
	t.Run("Publish --send-on newline from stdin", func(t *testing.T) {
		...
		out := runNatsCliWithInput(t, "test\npub\ninput", fmt.Sprintf("--server='%s' pub --send-on newline --force-stdin test", srv.ClientURL()))

		// Input should be split on newline (3 lines)
		lines := strings.Split(strings.TrimSpace(string(out)), "\n")
		if len(lines) != 4 {
	...
	t.Run("Publish --send-on eof from stdin", func(t *testing.T) {
		// --force-stdin required for testing as a terminal is not present
		out := runNatsCliWithInput(t, "test\npub\ninput", fmt.Sprintf("--server='%s' pub --force-stdin test", srv.ClientURL()))

		// Input should not be split on a newline (1 line)
		lines := strings.Split(strings.TrimSpace(string(out)), "\n")
		if len(lines) != 2 {
			t.Errorf("expected 2 lines got %d", len(lines))
		}
