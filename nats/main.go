...
`)
	...
	ncli.Flag("nkey", "User NKEY - Seed for nkey auth mode (when no JWT is provided).").Envar("NATS_NKEY").PlaceHolder("LOCATION").StringVar(&opts.Nkey)
	ncli.Flag("jwt", "User JWT - Use with seed").Envar("NATS_JWT").PlaceHolder("LOCATION").StringVar(&opts.UserJwt)
	ncli.Flag("seed", "User seed - Used in conjunction with JWT").Envar("NATS_SEED").PlaceHolder("LOCATION").StringVar(&opts.UserSeed)
