...
func configureAuthAccountCommand(auth commandHost) {
	...
	addCreateFlags := func(f *fisk.CmdClause, edit bool) {
		...
		f.Flag("js-memory-stream", "Sets the maximum size a Memory Storage stream may be").Default("-1").StringVar(&c.memMaxStreamString)
		...
		f.Flag("payload", "Maximum allowed payload").Default("-1").StringVar(&c.maxPayloadString)
