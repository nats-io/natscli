...
func (c *SrvReportCmd) dataSource() (serverdata.DataSource, error) {
	...
	return serverdata.NewServer(c.nc, func(req any, subj string, waitFor int, nc *nats.Conn) ([][]byte, error) {
...
func (c *SrvReportCmd) getConnz(limit int, nc *nats.Conn) (connzList, error) {
	...
	if !c.skipDiscoverClusterSize && c.waitFor == 0 && c.archivePath == "" {
		c.waitFor, err = serverdata.CurrentActiveServers(ctx, nc, opts().Timeout, opts().Trace)
		if err != nil {
			return nil, err
		}
	}
