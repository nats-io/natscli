...
func configureServerReportCommand(srv *fisk.CmdClause) {
	...
	reportCmd := report.Command("downgrade", "List assets incompatible with the specified API level").Action(c.downgradeAction)
...
func getServerAccounts(nc *nats.Conn) ([]*server.AccountDetail, error) {
	...
	accounts := []*server.AccountDetail{}
	accountMap := map[string]*server.AccountDetail{}

	for _, m := range resp {
		var serverInfo server.ServerAPIJszResponse
		if err := json.Unmarshal(m, &serverInfo); err != nil {
			return nil, err
		}

		for _, acct := range serverInfo.Data.AccountDetails {
			name := acct.Name
			if existing, found := accountMap[name]; found {
				existing.Streams = mergeStreams(existing.Streams, acct.Streams)
			} else {
				copy := *acct
				accountMap[name] = &copy
			}
		}
	}
	...
	// Sort accounts and streams
	sort.Slice(accounts, func(i, j int) bool {
		return accounts[i].Name < accounts[j].Name
	})

	for _, acct := range accounts {
		sort.Slice(acct.Streams, func(i, j int) bool {
			return acct.Streams[i].Name < acct.Streams[j].Name
		})
	}
...
func (c *SrvReportCmd) downgradeAction(_ *fisk.ParseContext) error {
