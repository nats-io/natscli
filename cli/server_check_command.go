...
When set these settings will be used, but can be overridden using --lag-critical.`)
	...
	stream.Flag("min-sources", "Minimum number of sources to expect").Default("1").IsSetByUser(&c.sourcesMinSourcesIsSet).IntVar(&c.sourcesMinSources)
	stream.Flag("max-sources", "Maximum number of sources to expect").Default("1").IsSetByUser(&c.sourcesMaxSourcesIsSet).IntVar(&c.sourcesMaxSources)
	stream.Flag("peer-expect", "Number of cluster replicas to expect").Default("1").IsSetByUser(&c.raftExpectIsSet).IntVar(&c.raftExpect)
