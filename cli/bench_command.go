...
`
	...
	bench.Flag("js", "Use JetStream").Default("false").UnNegatableBoolVar(&c.js)
	bench.Flag("request", "Request-Reply mode: publishers send requests waits for a reply").Default("false").UnNegatableBoolVar(&c.request)
	bench.Flag("reply", "Request-Reply mode: subscribers send replies").Default("false").UnNegatableBoolVar(&c.reply)
	bench.Flag("kv", "KV mode, subscribers get from the bucket and publishers put in the bucket").Default("false").UnNegatableBoolVar(&c.kv)
	...
	bench.Flag("no-progress", "Disable progress bar while publishing").Default("false").UnNegatableBoolVar(&c.noProgress)
	bench.Flag("csv", "Save benchmark data to CSV file").StringVar(&c.csvFile)
	bench.Flag("purge", "Purge the stream before running").Default("false").UnNegatableBoolVar(&c.purge)
	...
	bench.Flag("syncpub", "Synchronously publish to the stream").Default("false").UnNegatableBoolVar(&c.syncPub)
	bench.Flag("pubbatch", "Sets the batch size for JS asynchronous publishing").Default("100").IntVar(&c.pubBatch)
	bench.Flag("pull", "Use a shared durable explicitly acknowledged JS pull consumer rather than individual ephemeral consumers").Default("false").UnNegatableBoolVar(&c.pull)
	bench.Flag("push", "Use a shared durable explicitly acknowledged JS push consumer with a queue group rather than individual ephemeral consumers").Default("false").UnNegatableBoolVar(&c.pushDurable)
	...
	bench.Flag("multisubject", "Multi-subject mode, each message is published on a subject that includes the publisher's message sequence number as a token").Default("false").UnNegatableBoolVar(&c.multiSubject)
