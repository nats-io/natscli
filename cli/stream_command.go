...
func configureStreamCommand(app commandHost) {
	...
	addCreateFlags := func(f *fisk.CmdClause, edit bool) {
		...
		if !edit {
			...
			f.Flag("persist-mode", "Filestore persistence mode. Default is async for replicated streams and sync for replica=1. Async can only be forced for replica=1.").EnumVar(&c.persistMode, "default", "async")
		}
